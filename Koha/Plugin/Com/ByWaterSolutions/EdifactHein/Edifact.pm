package Koha::Plugin::Com::ByWaterSolutions::EdifactHein::Edifact;
...
use Koha::Plugin::Com::ByWaterSolutions::EdifactHein::Edifact::Segment;
use Koha::Plugin::Com::ByWaterSolutions::EdifactHein::Edifact::Message;
...
sub message_array {
    ...
    foreach my $seg ( @{ $self->{transmission} } ) {
        if ( $seg->tag eq 'UNH' ) {
            ...
            $in_msg = 0;
            if ( @{$msg} ) {
                push @{$msg_arr}, Koha::Plugin::Com::ByWaterSolutions::EdifactHein::Edifact::Message->new($msg);
                $msg = [];
            }
        }
        elsif ($in_msg) {
...
(?>         # dont backtrack into this group
    ...
    my @segmented;
    while ( $raw =~ /($re)/g ) {
        push @segmented, Koha::Plugin::Com::ByWaterSolutions::EdifactHein::Edifact::Segment->new( { seg_string => $1 } );
    }
    return \@segmented;
}
